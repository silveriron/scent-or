...
def generate_teacher_oof_with_smote(X, y, seed, suffix=""):
    ...
    print(f"   -> [Teacher] Training XGB{suffix} with SMOTE-Tomek...")

    for fold, (train_idx, val_idx) in enumerate(tqdm(kfold.split(X, y), total=N_SPLITS, desc=f"   xgb{suffix}")):
        ...
        save_path = os.path.join(TEACHER_SAVE_DIR, f"xgb_seed{seed}{suffix}_fold{fold+1}.model")
...
for SEED in SEEDS:
    ...
    print(f"\n[PHASE 1] Training Semantic-Only Teacher (XGB)...")
    oof_probs_sem = generate_teacher_oof_with_smote(X_semantic_only, labels, SEED, suffix="_sem")

    print(f"\n[PHASE 2] Training Hybrid Modality Teacher (XGB)...")
